import { createFileRoute, redirect } from "@tanstack/react-router";
import { useState } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import {
  useRequestMagicLink,
  type AuthError,
  type AuthFieldError,
} from "@/features/auth/hooks";

export const Route = createFileRoute("/magic-link")({
  beforeLoad: ({ context }) => {
    if (context.auth?.isAuthenticated) {
      throw redirect({ to: "/dashboard" });
    }
  },
  component: MagicLinkRequestRoute,
});

const requestMagicLinkSchema = z.object({
  email: z.string().min(1, "Email is required").email("Enter a valid email address"),
});

type RequestMagicLinkValues = z.infer<typeof requestMagicLinkSchema>;

export function MagicLinkRequestRoute() {
  const requestMagicLink = useRequestMagicLink();
  const [submitted, setSubmitted] = useState(false);

  const {
    register,
    handleSubmit,
    setError,
    formState: { errors, isSubmitting },
  } = useForm<RequestMagicLinkValues>({
    resolver: zodResolver(requestMagicLinkSchema),
    defaultValues: { email: "" },
  });

  async function onSubmit(values: RequestMagicLinkValues) {
    try {
      await requestMagicLink.mutateAsync(values.email);
      setSubmitted(true);
    } catch (rawError) {
      const err = rawError as AuthError | undefined;
      if (err?.kind === "rate_limited") {
        setError("root", { message: err.message });
        return;
      }
      setSubmitted(true);
    }
  }

  const rateLimitMessage = resolveRateLimitMessage(requestMagicLink.error);

  if (submitted) {
    return (
      <main className="mx-auto max-w-md px-6 py-12">
        <h1
          data-route-heading
          tabIndex={-1}
          className="text-3xl font-bold tracking-tight"
        >
          Check your email
        </h1>
        <p className="mt-4 text-sm text-muted-foreground">
          If that email has an account with us, we&apos;ve sent a sign-in link.
        </p>
        <p className="mt-4 text-sm text-muted-foreground">
          <a
            href="/sign-in"
            className="font-medium text-primary underline-offset-4 hover:underline"
          >
            Use a password instead
          </a>
        </p>
      </main>
    );
  }

  return (
    <main className="mx-auto max-w-md px-6 py-12">
      <h1
        data-route-heading
        tabIndex={-1}
        className="text-3xl font-bold tracking-tight"
      >
        Sign in with a magic link
      </h1>
      <p className="mt-2 text-sm text-muted-foreground">
        Enter your email and we&apos;ll send you a link that signs you in — no password
        required.
      </p>
      <form
        noValidate
        onSubmit={handleSubmit(onSubmit)}
        className="mt-8 space-y-4"
        aria-describedby={rateLimitMessage ? "magic-link-form-error" : undefined}
      >
        {rateLimitMessage ? (
          <p
            id="magic-link-form-error"
            role="alert"
            data-testid="auth-form-error"
            className="rounded-md border border-destructive bg-destructive/10 px-3 py-2 text-sm text-destructive"
          >
            {rateLimitMessage}
          </p>
        ) : null}
        <div className="space-y-2">
          <Label htmlFor="magic-link-email">Email</Label>
          <Input
            id="magic-link-email"
            type="email"
            autoComplete="email"
            aria-invalid={errors.email ? true : undefined}
            aria-describedby={errors.email ? "magic-link-email-error" : undefined}
            {...register("email")}
          />
          {errors.email ? (
            <p id="magic-link-email-error" className="text-sm text-destructive">
              {errors.email.message}
            </p>
          ) : null}
        </div>
        <Button type="submit" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? "Sending…" : "Send sign-in link"}
        </Button>
      </form>
    </main>
  );
}

function resolveRateLimitMessage(mutationError: unknown): string | null {
  const err = mutationError as AuthError | null | undefined;
  if (!err || typeof err !== "object" || !("kind" in err)) return null;
  if (err.kind !== "rate_limited") return null;
  const topLevel = Array.isArray(err.fieldErrors)
    ? err.fieldErrors.find((fieldError: AuthFieldError) => fieldError.field === null)
    : undefined;
  return topLevel?.message ?? err.message;
}
