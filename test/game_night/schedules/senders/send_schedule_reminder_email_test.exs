defmodule GameNight.Schedules.Senders.SendScheduleReminderEmailTest do
  @moduledoc """
  T119 — asserts the rendered subject is
  "Reminder: <gm_display_name> is waiting on your availability for <Month Year>"
  with the GM display name pulled from the notification payload
  rather than re-queried at send time.
  """
  use GameNight.DataCase, async: false

  import Swoosh.TestAssertions

  alias GameNight.Accounts.User
  alias GameNight.Games.{Game, Player}
  alias GameNight.Schedules.{Schedule, ScheduleParticipant}

  require Ash.Query

  test "subject reads 'Reminder: <gm_display_name> is waiting on your availability for <Month Year>'" do
    {:ok, gm} = create_user("gm")
    {:ok, game} = register_game(gm)
    {:ok, member} = create_user("member")
    _ = seed_player!(game, member)

    {:ok, schedule} =
      Schedule
      |> Ash.Changeset.for_create(
        :initiate,
        %{
          month: 10,
          year: 2099,
          start_time: ~T[19:00:00],
          end_time: ~T[23:00:00],
          time_zone: "America/Chicago",
          game_id: game.id
        },
        actor: gm
      )
      |> Ash.create()

    {:ok, schedule} =
      schedule
      |> Ash.Changeset.for_update(:set_gm_day, %{day: 5, status: :A}, actor: gm)
      |> Ash.update()

    {:ok, _ready} =
      schedule
      |> Ash.Changeset.for_update(:transition_to_ready_for_availability, %{}, actor: gm)
      |> Ash.update()

    [participant] =
      ScheduleParticipant
      |> Ash.Query.filter(schedule_id == ^schedule.id and player.user_id == ^member.id)
      |> Ash.read!(authorize?: false)

    drain_emails()

    {:ok, _} =
      participant
      |> Ash.Changeset.for_update(:send_reminder, %{}, actor: gm)
      |> Ash.update()

    expected_gm_name = to_string(gm.email)
    schedule_name = "October 2099 7:00 PM – 11:00 PM"

    assert_email_sent(fn email ->
      email.subject ==
        "Reminder: #{expected_gm_name} is waiting on your availability for #{schedule_name}"
    end)
  end

  defp create_user(prefix) do
    email =
      "schedule-reminder-email-#{prefix}-#{:erlang.unique_integer([:positive])}@example.test"

    User
    |> Ash.Changeset.for_create(:register_with_password, %{
      email: email,
      password: "schedule-reminder-email-test-password-1",
      password_confirmation: "schedule-reminder-email-test-password-1"
    })
    |> Ash.create(authorize?: false)
  end

  defp register_game(owner) do
    Game
    |> Ash.Changeset.for_create(:register, %{title: "Reminder email test", status: :active},
      actor: owner
    )
    |> Ash.create()
  end

  defp seed_player!(game, user) do
    Player
    |> Ash.Changeset.for_create(:create, %{
      game_id: game.id,
      user_id: user.id,
      character_name: "Test Char",
      status: :active
    })
    |> Ash.create!(authorize?: false)
  end

  defp drain_emails do
    receive do
      {:email, _} -> drain_emails()
    after
      0 -> :ok
    end
  end
end
