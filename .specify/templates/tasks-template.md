...
story begins. The `Tests for User Story N` phase is required, not
optional.
...
- **GameNight default**: Backend (Elixir/Phoenix/Ash) at repo root —
  source in `lib/`, tests in `test/`. Frontend (React/TanStack/Shadcn)
  in `assets/` — source in `assets/js/`, styles in `assets/css/`.
- Paths in sample tasks below use the GameNight default — adjust if a
  feature introduces a different layout per plan.md.
...
- [ ] TXXX [P] Additional unit tests in tests/unit/ or assets/test/unit/
