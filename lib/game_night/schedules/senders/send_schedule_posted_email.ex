defmodule GameNight.Schedules.Senders.SendSchedulePostedEmail do
  @moduledoc """
  Sends the "schedule posted" email to a linked player when the GM
  transitions a schedule to `:posted`.
  """
  ...
  defp subject_for(payload) do
    "#{payload["game_name"]} schedule for #{payload["schedule_name"]} has been posted"
  end
  ...
  defp body(payload, participant) do
    ...
    <p>
      ...
      <em>#{schedule_name}</em> &mdash; has been posted. The GM has
      finalised the days the game will run.
    </p>
    <p>
      <a href="#{character_url}">Open your character to view the posted days</a>.
    </p>
